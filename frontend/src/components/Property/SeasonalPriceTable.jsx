...
const SeasonalPriceTable = ({ seasonalPricing = [] }) => {
  ...
  const formatPrice = (price, pricingType) => {
    ...
    // Для per_period не добавляем "/ ночь"
    if (pricingType === 'per_period') {
  ...
  const getPriceLabel = (pricingType) => {
    if (pricingType === 'per_period') {
      return t('property.pricing.priceForPeriod')
    }
    return t('property.pricing.pricePerNight')
  }
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-5">
      <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-4 flex items-center space-x-2">
        ...
        <span>{t('property.pricing.seasonalPrices')}</span>
      </h3>
          ...
          <thead>
            <tr className="border-b border-gray-200 dark:border-gray-700">
              ...
              <th className="text-right py-2 px-3 text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase">
                {t('property.pricing.price')}
              </th>
            </tr>
          </thead>
          <tbody>
            {displayedPricing.map((period, index) => {
              const pricingType = period.pricing_type || 'per_night'
              return (
                <motion.tr
                  key={index}
                  ...
                  <td className="py-3 px-3 text-right">
                    <div className="flex flex-col items-end">
                      <span className="text-lg font-bold text-gray-900 dark:text-white">
                        {formatPrice(period.price_per_night, pricingType)}
                      </span>
                      <span className="text-xs text-gray-500 dark:text-gray-400">
                        {getPriceLabel(pricingType)}
                      </span>
                    </div>
                  </td>
                </motion.tr>
      ...
      <div className="mt-4 p-3 bg-blue-50/50 dark:bg-blue-900/10 rounded-lg border border-blue-200/50 dark:border-blue-800/30">
        <p className="text-xs text-gray-600 dark:text-gray-400">
          ...
          {t('property.pricing.seasonalNoteText')}
        </p>
      </div>
    </div>
  )

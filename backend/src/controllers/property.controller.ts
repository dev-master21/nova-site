...
async getComplexProperties(req: AuthRequest, res: Response) {
  try {
    ...
    console.log(`🏘️ Получение объектов комплекса: ${complexName}, язык: ${language}, исключить ID: ${excludeId || 'нет'}`);

    // Базовый запрос с расширенными полями
    let query = `
      SELECT 
        p.id,
        p.property_number,
    ...
    const params: any[] = [language, complexName];

    // Опционально исключаем текущий объект
    if (excludeId) {
      query += ` AND p.id != ?`;
      params.push(excludeId);
    }

    query += ` ORDER BY p.property_number ASC`;

    const properties: any = await db.query(query, params);

    console.log(`📦 Найдено ${properties.length} объектов в комплексе`);

    // Обрабатываем каждый объект - добавляем цены и форматируем данные
    const processedProperties = await Promise.all(properties.map(async (property: any) => {
      ...
      // Получаем минимальную цену за ночь
      const minPriceResult: any = await db.query(
        `SELECT MIN(price_per_night) as min_price 
         FROM property_pricing 
         WHERE property_id = ? 
           AND price_per_night > 0
           AND (pricing_type = 'per_night' OR pricing_type IS NULL)`,
        [property.id]
      );
      ...
      return {
        ...
        year_price: property.year_price ? parseFloat(property.year_price) : null,
        min_price: minPriceResult[0]?.min_price ? parseFloat(minPriceResult[0].min_price) : null,
        // Ownership info для продажи
        ...
        photos: photos,
        // Ценообразование
        seasonalPricing: seasonalPricing.map((p: any) => ({
          start_date_recurring: p.start_date_recurring,
          end_date_recurring: p.end_date_recurring,
          price_per_night: p.price_per_night ? parseFloat(p.price_per_night) : null,
          season_type: p.season_type,
          pricing_type: p.pricing_type
        })),
        monthlyPricing: monthlyPricing.map((p: any) => ({
          month_number: p.month_number,
          price_per_month: p.price_per_month ? parseFloat(p.price_per_month) : null
        }))
      };
    }));

    console.log(`✅ Обработано ${processedProperties.length} объектов в комплексе`);
      ...
      try {
        ...
        const query = `
          SELECT 
            ...
            p.complex_name,
            pt.property_name as name,
            (SELECT MIN(price_per_night) FROM property_pricing WHERE property_id = p.id AND price_per_night > 0 AND (pricing_type = 'per_night' OR pricing_type IS NULL)) as price_per_night,
            (
              SELECT GROUP_CONCAT(
                pp2.photo_url
                ORDER BY pp2.is_primary DESC, pp2.sort_order ASC
        ...
        const processedProperties = await Promise.all(properties.map(async (property: any) => {
          ...
          let complex_min_sale_price = null;
          let min_price_property_id = property.id;
          let min_sale_price_property_id = property.id;
          ...
          if (property.complex_name) {
            ...
            // Минимальная цена аренды комплекса И ID объекта с минимальной ценой
            const complexPricingResult: any = await db.query(
              `SELECT 
               ...
               WHERE p.complex_name = ?
                 ...
                 AND (pricing.pricing_type = 'per_night' OR pricing.pricing_type IS NULL)
            ...
            // Минимальная цена продажи комплекса
            const complexSalePricingResult: any = await db.query(
              `SELECT 
                 p.sale_price as min_sale_price,
                 p.id as min_sale_price_property_id
               FROM properties p
               WHERE p.complex_name = ?
                 AND p.status = 'published'
                 AND p.deleted_at IS NULL
                 AND p.sale_price > 0
                 AND (p.deal_type = 'sale' OR p.deal_type = 'both')
               ORDER BY p.sale_price ASC
               LIMIT 1`,
              [property.complex_name]
            );
            complex_min_sale_price = complexSalePricingResult[0]?.min_sale_price || null;
            min_sale_price_property_id = complexSalePricingResult[0]?.min_sale_price_property_id || property.id;
          }
        
          return {
            id: property.id,
            min_price_property_id: min_price_property_id,
            min_sale_price_property_id: min_sale_price_property_id,
            name: property.name || `Property ${property.id}`,
            property_number: property.property_number,
            property_type: property.property_type,
            ...
            complex_min_sale_price: complex_min_sale_price,
            seasonalPricing: seasonalPricing.map((p: any) => ({
              start_date_recurring: p.start_date_recurring,
              end_date_recurring: p.end_date_recurring,
              price_per_night: p.price_per_night ? parseFloat(p.price_per_night) : null,
...
property.monthlyPricing = monthlyPricing;

// Получаем годовую цену
property.yearPrice = property.year_price || null;
...
/**
 * Расчет стоимости проживания за период (ПОЛНОСТЬЮ ПЕРЕРАБОТАН)
 */
async calculatePrice(req: Request, res: Response) {
  try {
    ...
    // Вспомогательная функция: количество дней в месяце
    const getDaysInMonth = (year: number, month: number): number => {
      return new Date(year, month, 0).getDate();
    };

    // Вспомогательная функция: проверка на полный календарный месяц
    const isFullCalendarMonth = (startDate: Date, endDate: Date): boolean => {
      // Проверяем: начало с 1-го числа и конец на 1-е число следующего месяца
      if (startDate.getDate() !== 1) return false;
      if (endDate.getDate() !== 1) return false;
      
      // Проверяем что это следующий месяц
      const expectedEndMonth = startDate.getMonth() + 1;
      const expectedEndYear = startDate.getFullYear() + (expectedEndMonth > 11 ? 1 : 0);
      const normalizedEndMonth = expectedEndMonth % 12;
      
      return endDate.getMonth() === normalizedEndMonth && endDate.getFullYear() === expectedEndYear;
    };
    ...
    const property = properties[0];
    const yearPrice = property.year_price ? parseFloat(property.year_price) : null;
    ...
    console.log(`📊 Доступные цены:`);
    console.log(`   - Годовая: ${yearPrice ? '฿' + yearPrice : 'нет'}`);
    console.log(`   - Месячных периодов: ${monthlyPricing.length}`);
    console.log(`   - Сезонных периодов: ${seasonalPricing.length}`);
    ...
    console.log(`📅 Доступность: ${isAvailable ? 'Свободен' : `Занято ${unavailableDates.size} дней`}`);
    console.log(`📅 Список занятых дат:`, Array.from(unavailableDates).sort());

    // ============ ЛОГИКА РАСЧЕТА ЦЕНЫ ============

    let totalPrice = 0;
    let priceBreakdown: any[] = [];
    let pricingType: string = 'seasonal';
    let isApproximate = false;
    let disclaimer = '';
    let availableOnlyForYearly = false;

    // ПРИОРИТЕТ 1: Годовой контракт (>= 330 дней ≈ 11 месяцев)
    if (nights >= 330) {
      console.log(`\n🏆 ПРИОРИТЕТ 1: Проверка годового контракта (${nights} дней >= 330)`);
      
      if (yearPrice && yearPrice > 0) {
        pricingType = 'yearly';
        
        // Считаем помесячно с реальным количеством дней
        ...
        while (currentDate < checkOutDate) {
          ...
          // Определяем конец периода в этом месяце
          ...
          // Проверяем - это полный календарный месяц?
          const isFullMonth = isFullCalendarMonth(periodStart, periodEnd);
          
          let periodPrice: number;
          let daysInPeriod: number;
          
          if (isFullMonth) {
            // Полный месяц = полная месячная цена
            periodPrice = yearPrice;
            daysInPeriod = daysInThisMonth;
            console.log(`   📅 Полный месяц ${currentMonth}/${currentYear}: ฿${periodPrice} (${daysInPeriod} дней)`);
          } else {
            // Частичный месяц = пропорционально
            daysInPeriod = Math.ceil((periodEnd.getTime() - periodStart.getTime()) / (1000 * 60 * 60 * 24));
            const pricePerDay = yearPrice / daysInThisMonth;
            periodPrice = pricePerDay * daysInPeriod;
            console.log(`   📅 Частичный месяц ${currentMonth}/${currentYear}: ${daysInPeriod} дней × ฿${pricePerDay.toFixed(2)}/день = ฿${periodPrice.toFixed(0)}`);
          }
          ...
          priceBreakdown.push({
            date: dateStr,
            price: Math.round(periodPrice),
            seasonType: 'yearly',
            isZeroPrice: false,
            isOccupied: unavailableDates.has(dateStr),
            periodType: 'month',
        ...
        console.log(`✅ Используем годовую цену: ฿${yearPrice}/месяц, итого: ฿${totalPrice.toFixed(0)}`);
      } else {
        console.log(`⚠️ Годовая цена не указана, но период >= 11 месяцев`);
        availableOnlyForYearly = true;
        pricingType = 'unavailable';
        disclaimer = 'yearlyOnly';
      }
    }
    
    // ПРИОРИТЕТ 2: Месячные цены (>= 28 дней и < 330 дней)
    else if (nights >= 28 && pricingType === 'seasonal' && monthlyPricing.length > 0) {
      console.log(`\n🏆 ПРИОРИТЕТ 2: Используем месячные цены (${nights} дней >= 28)`);
      
      let currentDate = new Date(checkInDate);
      let hasAllMonthlyPrices = true;
      ...
      while (currentDate < checkOutDate) {
        ...
        console.log(`\n📅 Обработка месяца ${currentMonth}/${currentYear} (всего ${daysInThisMonth} дней в месяце)`);
        
        const monthPrice = monthlyPricing.find((m: any) => m.month_number === currentMonth);
        
        if (monthPrice && monthPrice.price_per_month) {
          const pricePerMonth = parseFloat(monthPrice.price_per_month);
          ...
          if (isFullMonth) {
            // Полный месяц = полная месячная цена
            periodPrice = pricePerMonth;
            daysInPeriod = daysInThisMonth;
            console.log(`   ✅ Полный календарный месяц: ฿${pricePerMonth} за весь месяц`);
          } else {
            // Частичный месяц = пропорционально по дням
            daysInPeriod = Math.ceil((periodEnd.getTime() - periodStart.getTime()) / (1000 * 60 * 60 * 24));
            const pricePerDay = pricePerMonth / daysInThisMonth;
            periodPrice = pricePerDay * daysInPeriod;
            console.log(`   ✅ Частичный месяц: ${daysInPeriod} из ${daysInThisMonth} дней`);
            console.log(`   💰 Расчет: ฿${pricePerMonth} / ${daysInThisMonth} дней × ${daysInPeriod} = ฿${periodPrice.toFixed(0)}`);
          }
          ...
          console.log(`   ❌ Месячная цена для месяца ${currentMonth} не найдена`);
          hasAllMonthlyPrices = false;
          break;
        }
      ...
      if (hasAllMonthlyPrices) {
        pricingType = 'monthly';
        console.log(`✅ Расчет по месячным ценам завершен: ฿${totalPrice.toFixed(0)}`);
      } else {
        console.log(`⚠️ Не все месяцы имеют цены, переходим к сезонным ценам`);
        totalPrice = 0;
        priceBreakdown = [];
      }
    }
    
    // ПРИОРИТЕТ 3: Сезонные цены (если не использовали годовые/месячные)
    if (pricingType === 'seasonal' && !availableOnlyForYearly) {
      console.log(`\n🏆 ПРИОРИТЕТ 3: Используем сезонные цены`);
      ...
      const perNightPrices = seasonalPricing.filter((p: any) => 
        p.pricing_type === 'per_night' || !p.pricing_type
      );
      
      if (perNightPrices.length > 0) {
        console.log(`   ✅ Найдено ${perNightPrices.length} периодов per_night`);
        ...
        while (currentDate < checkOutDate) {
          ...
          for (const period of perNightPrices) {
      ...
      else if (monthlyPricing.length > 0 && nights < 28) {
        console.log(`   ⚠️ Нет сезонных per_night цен, но есть месячные. Используем примерный расчет.`);
        ...
        while (currentDate < checkOutDate) {
          ...
          if (monthPrice && monthPrice.price_per_month) {
            // Используем реальное количество дней в месяце
            const pricePerDay = parseFloat(monthPrice.price_per_month) / daysInThisMonth;
            totalPrice += pricePerDay;
            
            priceBreakdown.push({
              date: dateStr,
              price: Math.round(pricePerDay),
              seasonType: 'approximate',
              isZeroPrice: false,
              isApproximate: true,
              isOccupied: unavailableDates.has(dateStr),
              daysInMonth: daysInThisMonth
        ...
        pricingType = 'approximate';
        isApproximate = true;
        disclaimer = 'approximateMonthly';
      }
      else {
        console.log(`   ❌ Нет доступных цен для расчета`);
        ...
        if (yearPrice && yearPrice > 0) {
          ...
          hasZeroPriceDays = true;
          pricingType = 'unavailable';
          disclaimer = 'noPricing';
        }
      }
      
      if (hasZeroPriceDays && !isApproximate && !availableOnlyForYearly) {
        console.log(`   ⚠️ Обнаружены дни с нулевой ценой`);
      }
    }
    ...
    const result = {
      ...
      isApproximate,
      disclaimer,
      availableOnlyForYearly,
      hasZeroPriceDays: priceBreakdown.some(d => d.isZeroPrice)
    };

    console.log(`\n✅ ИТОГО: ฿${result.totalPrice} за ${nights} ночей (тип: ${pricingType})`);
    console.log(`📊 Breakdown содержит ${priceBreakdown.length} записей`);
    console.log(`📊 Занятых дней в breakdown: ${priceBreakdown.filter(d => d.isOccupied).length}`);
...
// Ищем подходящий период только для pricing_type = 'per_night'
let foundPrice = null;
for (const period of pricing) {
  // Учитываем только цены за ночь
  if (period.pricing_type === 'per_night' || !period.pricing_type) {
    if (isDateInRange(day, month, period.start_date_recurring, period.end_date_recurring)) {
...
// Если не нашли подходящий период среди per_night, берем минимальную per_night цену
if (foundPrice === null) {
  const perNightPrices = pricing
    .filter((p: any) => p.pricing_type === 'per_night' || !p.pricing_type)
    .map((p: any) => parseFloat(p.price_per_night))
    .filter((price: number) => price > 0);
  
  if (perNightPrices.length > 0) {
    foundPrice = Math.min(...perNightPrices);
    console.log(`⚠️ Период не найден, используем минимальную per_night цену: ฿${foundPrice}`);
  }
}
...
async getVillasForPage(req: Request, res: Response) {
  try {
    const { 
      ...
      bedrooms, 
      name,
      dealType = 'rent', // 'rent' или 'sale'
      propertyTypes, // 'house,villa,condo' - через запятую
      page = 1, 
      limit = 12 
    } = req.query;

    console.log('🏠 Загрузка вилл для страницы');
    console.log('Параметры:', { checkIn, checkOut, bedrooms, name, dealType, propertyTypes, page, limit });

    // ========= ДИАГНОСТИКА - НАЧАЛО =========
    const allPublished: any = await db.query(
      `SELECT COUNT(*) as count FROM properties WHERE status = 'published' AND deleted_at IS NULL`
    );
    console.log('📊 Всего опубликованных объектов:', allPublished[0]?.count);

    const byDealType: any = await db.query(
      `SELECT deal_type, COUNT(*) as count 
       FROM properties 
       WHERE status = 'published' AND deleted_at IS NULL 
       GROUP BY deal_type`
    );
    console.log('📊 По типу сделки:', byDealType);

    const byPropertyType: any = await db.query(
      `SELECT property_type, COUNT(*) as count 
       FROM properties 
       WHERE status = 'published' AND deleted_at IS NULL 
       GROUP BY property_type`
    );
    console.log('📊 По типу недвижимости:', byPropertyType);
    // ========= ДИАГНОСТИКА - КОНЕЦ =========
...
// Базовый запрос
    ...
    // ========= ИСПРАВЛЕННЫЙ ФИЛЬТР ПО ТИПУ СДЕЛКИ =========
    // Учитываем NULL как "доступен для аренды" (обратная совместимость)
    if (dealType === 'rent') {
      // Для аренды: rent, both, или NULL (старые объекты без указанного типа)
      query += ` AND (p.deal_type = 'rent' OR p.deal_type = 'both' OR p.deal_type IS NULL OR p.deal_type = '')`;
    } else if (dealType === 'sale') {
      // Для продажи: только sale или both (NULL не включаем - старые объекты обычно для аренды)
      query += ` AND (p.deal_type = 'sale' OR p.deal_type = 'both')`;
    }

    // ========= ИСПРАВЛЕННЫЙ ФИЛЬТР ПО ТИПАМ НЕДВИЖИМОСТИ =========
    if (propertyTypes) {
      const types = (propertyTypes as string).split(',').filter(t => t.trim());
      
      // Если выбраны все 3 типа - НЕ применяем фильтр (показываем всё)
      const allTypesSelected = types.length >= 3 || 
        (types.includes('house') && types.includes('villa') && types.includes('condo'));
      
      if (!allTypesSelected && types.length > 0) {
        const expandedTypes: string[] = [];
        
        types.forEach(type => {
          if (type === 'house') {
            expandedTypes.push('house');
          } else if (type === 'villa') {
            expandedTypes.push('villa');
          } else if (type === 'condo') {
            // Condo включает condo, apartment, penthouse
            expandedTypes.push('condo', 'apartment', 'penthouse');
          }
        });

        if (expandedTypes.length > 0) {
          // Также включаем объекты с NULL property_type (старые объекты)
          const placeholders = expandedTypes.map(() => '?').join(',');
          query += ` AND (p.property_type IN (${placeholders}) OR p.property_type IS NULL OR p.property_type = '')`;
          params.push(...expandedTypes);
        }
      }
      // Если все типы выбраны - фильтр не добавляем, показываем все объекты
      console.log(`📊 Фильтр по типам: ${allTypesSelected ? 'ВСЕ (без фильтра)' : types.join(', ')}`);
    }
    ...
    for (const property of properties) {
      ...
      property.seasonalPricing = seasonalPricing.map((p: any) => ({
        start_date_recurring: p.start_date_recurring,
        end_date_recurring: p.end_date_recurring,
        price_per_night: p.price_per_night ? parseFloat(p.price_per_night) : null,
        season_type: p.season_type,
      ...
      property.monthlyPricing = monthlyPricing.map((p: any) => ({
        month_number: p.month_number,
        price_per_month: p.price_per_month ? parseFloat(p.price_per_month) : null
      }));

      // Преобразуем числовые поля
      property.sale_price = property.sale_price ? parseFloat(property.sale_price) : null;
      property.year_price = property.year_price ? parseFloat(property.year_price) : null;

      // Получаем минимальную цену с учетом типа (для обратной совместимости)
      const pricing: any = await db.query(
        `SELECT MIN(price_per_night) as min_price 
         FROM property_pricing 
         WHERE property_id = ? 
      ...
      property.min_price = pricing[0]?.min_price ? parseFloat(pricing[0].min_price) : null;

      // Если объект входит в комплекс - получаем данные комплекса
      if (property.complex_name) {
        // Получаем минимальную цену аренды И ID объекта с этой ценой
        ...
        property.complex_min_price = complexPricing[0]?.complex_min_price ? parseFloat(complexPricing[0].complex_min_price) : null;
        property.min_price_property_id = complexPricing[0]?.min_price_property_id || property.id;

        // Получаем минимальную цену продажи И ID объекта
        const complexSalePricing: any = await db.query(
          `SELECT 
             p.sale_price as complex_min_sale_price,
             p.id as min_sale_price_property_id
           FROM properties p
           WHERE p.complex_name = ?
             AND p.status = 'published'
        ...
        property.complex_min_sale_price = complexSalePricing[0]?.complex_min_sale_price ? parseFloat(complexSalePricing[0].complex_min_sale_price) : null;
        property.min_sale_price_property_id = complexSalePricing[0]?.min_sale_price_property_id || property.id;
    ...
    if (!hasSearchParams) {
      ...
      const standaloneProperties: any[] = [];
    ...
    console.log('   - data.length:', finalProperties.length);
...
private async calculatePriceForPeriod(propertyId: number, checkIn: string, checkOut: string) {
  try {
    ...
    // Получаем месячные цены
    const monthlyPricing: any = await db.query(
      `SELECT month_number, price_per_month FROM property_pricing_monthly WHERE property_id = ?`,
      [propertyId]
    );

    if ((!pricing || pricing.length === 0) && (!monthlyPricing || monthlyPricing.length === 0)) {
      console.log(`⚠️ Нет ценовых данных для объекта #${propertyId}`);
      return null;
    }

    console.log(`📊 Найдено ${pricing.length} сезонных периодов, ${monthlyPricing.length} месячных`);
    ...
    while (currentDate < checkOutDate) {
      ...
      // Если сезонная цена не найдена, пробуем месячную
      if (priceForDay === null || isNaN(priceForDay)) {
        const monthPrice = monthlyPricing.find((m: any) => m.month_number === month);
        if (monthPrice && monthPrice.price_per_month) {
          // Используем реальное количество дней в месяце
          priceForDay = parseFloat(monthPrice.price_per_month) / daysInThisMonth;
          console.log(`    ✅ ${currentDDMM}: Месячная цена, ฿${monthPrice.price_per_month}/${daysInThisMonth} = ฿${priceForDay.toFixed(2)}/день`);
        }
      }

      // Fallback на минимальную цену
      if (priceForDay === null || isNaN(priceForDay)) {
        console.log(`    ⚠️ Цена не найдена для ${currentDDMM}`);
